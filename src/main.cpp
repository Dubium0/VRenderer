...
private:
    ...
    glm::vec3 m_position = glm::vec3(0.0f),
        m_rotation = glm::vec3(0.0f), m_scale = glm::vec3(1.0f);

    double m_lastX, m_lastY;
    bool m_firstMouse = true;
    bool m_onCameraMode = false;
    ...
    void initWindow() {
        ...
        glfwWindowHint(GLFW_CLIENT_API, GLFW_NO_API); // will set later on
        ...
        auto mouse_callback = [](GLFWwindow* w, double xpos, double ypos)
            {
                static_cast<VulkanRenderer*>(glfwGetWindowUserPointer(w))->onMouseMovement(w, xpos, ypos);
            };

        auto mouse_scroll_callback = [](GLFWwindow* w, double offsetx, double offsety)
            {
                static_cast<VulkanRenderer*>(glfwGetWindowUserPointer(w))->onMouseScroll(w, offsetx, offsety);
            };

        glfwSetCursorPosCallback(m_window, mouse_callback); // mouse position 
        glfwSetScrollCallback(m_window, mouse_scroll_callback); // scrolling
    ...
    void mainLoop() {
        ...
        while (!glfwWindowShouldClose(m_window)) {
            ...
            m_deltaTime = currentFrameTime - lastFrame; // time passed between two frames

            lastFrame = currentFrameTime;

            deltaSeconds = currentFrameTime - lastTime; // time passed between fps updates


            m_frameCount++;
            if (deltaSeconds >= 1) {

                auto frameRate = double(m_frameCount) / deltaSeconds;

                std::stringstream title;
                title << "Vulkan Renderer" << " FPS : " << frameRate;
    ...
    void initCamera() {
        m_camera.Position = glm::vec3(0.0f, 0.0f, 2.0f);
    }
    ...
    void drawFrame() {
        ...
        currentFrame = (currentFrame + 1) % MAX_FRAMES_IN_FLIGHT;// nice trick :O
    ...
    void createImage(uint32_t width, uint32_t height,
        uint32_t mipLevels, VkSampleCountFlagBits numSample, VkFormat format, VkImageTiling tiling,
    ...
    void updateUniformBuffer(uint32_t currentImage) {
        ...
        ubo.model = glm::translate(ubo.model, m_position);

        ubo.view = m_camera.GetViewMatrix();

        ubo.proj = glm::perspective(glm::radians(m_camera.Zoom), m_swapChainExtent.width / (float)m_swapChainExtent.height, 0.1f, 10000.0f);
    ...
    void loadModel() {
        ...
        auto min = 9999999999999.0f;
        auto max = -9999999999999.0f;
        ...
        glm::vec3 vectorMin = glm::vec3(0.0f);
        glm::vec3 vectorMax = glm::vec3(0.0f);
        ...
        for (const auto& shape : shapes) {
            for (const auto& index : shape.mesh.indices) {
                ...
                auto nextLenght = glm::length(vertex.pos);
                if (nextLenght > max) {
                    vectorMax = vertex.pos;
                    max = nextLenght;
                }
                if (nextLenght < min) {
                    vectorMin = vertex.pos;
                    min = nextLenght;
                }
                ...
                //std::cout << m_indices.back()<<std::endl;
            }

        }
        m_modelCenter = (vectorMax + vectorMin) / 2.0f;
        std::cout << " Model Center : " << m_modelCenter.x << ", " << m_modelCenter.y << ", " << m_modelCenter.z << '\n';
    ...
    void createGraphicsPipeline() {
        auto vShaderCode = readFile("shaders/bytecodes/vert.spv");
        auto fShaderCode = readFile("shaders/bytecodes/frag.spv");

        VkShaderModule vertShaderModule = createShaderModule(vShaderCode);
        VkShaderModule fragShaderModule = createShaderModule(fShaderCode);
    ...
    void createInstance() {
        ...
        //mandatory to create vulkan instance
    ...
    void createSwapChain() {
        ...
        if (swapChainSupport.capabilities.maxImageCount > 0 && imageCount > swapChainSupport.capabilities.maxImageCount) {
            imageCount = swapChainSupport.capabilities.maxImageCount; // zero means infinite
        }
        ...
        createInfo.preTransform = swapChainSupport.capabilities.currentTransform; // if you want to apply transformation to render image
    ...
    void pickPhysicalDevice() {
        ...
        if (candidates.rbegin()->first > 0) {
            ...
            std::cout << "Chosen Vulkan Device : " << deviceProperties.deviceName << std::endl;
    ...
    int rateDeviceSuitability(VkPhysicalDevice device) {
        ...
        score += deviceProperties.limits.maxImageDimension2D; // example of scoring
        return score;
    }
    ...
    void checkSupportedExtensions(const char** _extensions, int size) {
        std::vector<std::string> requiredExtensions(_extensions, _extensions + size);
        ...
        for (int i = 0; i < size; i++) {
            ...
            for (int k = 0; k < extensionCount; k++) {
                if (extensions[k].extensionName == requiredExtensions[i]) {
                    notFoundCount--;
                    res = "TRUE";
                    break;
                }
            }
            std::cout << '\t' << requiredExtensions[i] << " : " << res << '\n';

        }
        std::cout << "-----------------------------------------------------------" << std::endl;
        if (notFoundCount > 0) {
            std::cout << "Result : some extensions not found " << std::endl;
        }
        else {
            std::cout << "Result : all extensions available " << std::endl;
        }
        std::cout << "-----------------------------------------------------------" << std::endl;
    }
    ...
    void processInput() {
        ...
        {
            if (m_onCameraMode)
                m_camera.ProcessKeyboard(Camera_Movement::UPWARD, deltaTime);

        }
        if (glfwGetKey(m_window, GLFW_KEY_E) == GLFW_PRESS) {

            if (m_onCameraMode)
                m_camera.ProcessKeyboard(Camera_Movement::DOWNWARD, deltaTime);
